...
const Keyboard = (props) => {
  ...
  const keyboard = sectionKeys.map((sectionKey) => {
    const keyboardButtonEls = keyboardButtons[sectionKey].map(
      (sectionValue) => (
        <KeyboardButton
          key={sectionValue.id}
  ...
  return <section className={classes.keyboard}>{keyboard}</section>;
};
