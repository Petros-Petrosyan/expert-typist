...
// game status
import { IN_PROGRESS, INIT } from "../../constants/gameStatus";
...
const initialState = {
  ...
  textInfo: {
    text: "Click the keyboard to get the text",
    loading: false,
    exist: true,
    currentIndex: 0,
    writtenLetter: null,
...
const gameReducer = (state = initialState, action) => {
  ...
  switch (type) {
    ...
    case WRITE_LETTER:
      return writeLetter(state, payload);
    case CHANGE_CURRENT_INDEX:
      return changeCurrentIndex(state, payload);
    case COLLECT_ERRORS:
      return collectErrors(state, payload);
    case CHANGE_TIME:
      return changeTime(state, payload);

    case GET_TEXT_START:
      return getTextStart(state);
    case GET_TEXT_SUCCESS:
      return getTextSuccess(state, payload);
    case GET_TEXT_FAIL:
      return getTextFail(state);
